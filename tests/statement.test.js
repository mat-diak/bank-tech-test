...
describe("Statement", () => {
  ...
  describe("saveTransaction", () => {
    it("saves a withdrawal", () => {
      ...
      expect(statement.transactionHistory).toContainEqual(
        { date: new Date(), type: "withdrawal", amount: 500, balance: 500 },
      );
    });
  });
  ...
  describe('print', () => {
    it('returns the statement', () => {
      ...
      expect(stFormatter.formatFullStatement).toHaveBeenCalledWith(st.transactionHistory)
      expect(consoleSpy).toHaveBeenCalledWith(stFormatter.formatFullStatement())
    });
  });
});
